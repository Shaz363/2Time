...
export const createJob = async (req, res, next) => {
  try {
    ...
    const updateCompany = await Users.findByIdAndUpdate(id, company, {
      new: true,
    });
...
export const getJobById = async (req, res, next) => {
  try {
    ...
    const job = await Jobs.findById({ _id: id })
      .populate({
        path: "company",
        select: "name email accountType about location -password",
      })
      .populate({
        path: "application",
        match: { user: userId, job: id },
        select: "_id user job",
      });
...
export const getJobSuggestion = async (req, res, next) => {
  try {
    ...
    const result =
      search === "jobs"
        ? await Jobs.aggregate([
            ...
            { $project: { _id: 0, titles: 1, locations: 1 } },
          ])
