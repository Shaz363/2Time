...
function MenuList({ user, onClick }) {
  ...
  const handleLogout = () => {
    socket?.emit("end", { userId: user._id });
    dispatch(Logout());
        ...
        <nav className='container mx-auto flex items-center justify-between p-5'>
          <div>
            <Link to='/' className='text-blue-600 font-bold text-3xl'>
              2<span className='text-[#1677cccb]'>Time</span>
            </Link>
          </div>
          ...
          <ul className='hidden lg:flex gap-10 text-xl'>
            ...
            <li>
              <Link
                to='/direct-chats'
                className='relative w-20 h-7 flex gap-1 items-center border border-blue-500 px-2 py-0.5 rounded-full'
              >
                Chats <IoChatbubblesOutline />
                {unReadCount > 0 && (
                  <p className='absolute w-5 h-5 rounded-full bg-red-600 flex items-center justify-center text-white text-[10px] text-center -top-2 right-0'>
                    {unReadCount}
                  </p>
                )}
              </Link>
          ...
          <Link
            to='/direct-chats'
            className='relative w-30 h-7 flex md:hidden  items-center border border-blue-700 px-2 py-0.5 rounded-full'
          >
            Chats <IoChatbubblesOutline />
            {unReadCount > 0 && (
              <p className='absolute w-5 h-5 rounded-full bg-red-600 flex items-center justify-center text-white text-[10px] text-center -top-2 right-0'>
